...
public class ObjectManager {
     ...
     */
    public void placePerItemGames() {
        //RADIOS
        ArenaChest[] radioChests = getRandomizedItemChests();
        double numChestsToPlaceForRadios = Math.min(Math.round((arena.getGameManager().getPlayerManager().getNumberOfCounselors() + 1) / 2), radioChests.length);
        int i;
     ...
     * @param player
     */
    public void spawnCorpse(F13Player player) {
        Corpses.CorpseData corpse = CorpseAPI.spawnCorpse(player.getBukkitPlayer(), player.getBukkitPlayer().getLocation());
        corpses.add(corpse);
